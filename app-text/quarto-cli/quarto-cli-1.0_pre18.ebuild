...
if [[ "${PV}" == *9999 ]];then
	...
	#I'm worried the preview versions will go 1.0.n -> 1.0 but perhaps
	#they will go 1.0.n(preview) -> 1.0.n+1(release)
	MY_PV=$(ver_cut 1-2).$(ver_cut 4)
	S=${WORKDIR}/${PN}-${MY_PV}
	SRC_URI="https://github.com/quarto-dev/quarto-cli/archive/refs/tags/v${MY_PV}.tar.gz   -> ${P}.tar.gz"
fi
...
src_compile(){
	...
	if use bundle;then
		...
		echo -n "${MY_PV}"  > src/resources/version
	fi
