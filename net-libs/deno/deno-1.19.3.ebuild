...
CRATES="
	...
	deno-1.19.3
	deno_ast-0.12.1
	deno_bench_util-0.34.0
	deno_broadcast_channel-0.34.0
	deno_console-0.40.0
	deno_core-0.122.0
	deno_crypto-0.54.0
	deno_doc-0.32.0
	...
	deno_webgpu-0.41.0
	deno_webidl-0.40.0
	deno_websocket-0.45.0
	deno_webstorage-0.35.0
	der-0.4.5
	derive_more-0.99.17
	diff-0.1.12
	digest-0.9.0
...
inherit cargo llvm multiprocessing
...
#MIT OR Unlicense (7)
#MPL-2.0 (1)
#N/A (4): ring, test_ffi, test_util, webpki
#Zlib (1)

LICENSE="0BSD Apache-2.0 Apache-2.0-with-LLVM-exceptions Artistic-2 BSD BSD-2 Boost-1.0 CC0-1.0 ISC MIT MPL-2.0 Unlicense ZLIB"
SLOT="0"
KEYWORDS="~amd64"
BDEPEND="
	sys-devel/llvm
...
src_prepare() {
	default
	pushd "${WORKDIR}/cargo_home/gentoo/v8-0.40.2" > /dev/null
	patch -p1 < ${FILESDIR}/v8-0.40.2-jobfix.patch
	popd > /dev/null
	}
src_compile() {
	...
	let JOBS_HALF=$(makeopts_jobs)/2
	export NINJA_JOBS=${JOBS_HALF}
	einfo "Number of jobs:${NINJA_JOBS}"
	#GN_ARGS=
