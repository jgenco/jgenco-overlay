...
DENO_STD_VER="0.163.0"

SRC_URI="$(cargo_crate_uris)"
SRC_URI+=" https://github.com/denoland/deno/archive/refs/tags/v${PV}.tar.gz -> deno-${PV}.tar.gz
	test? (
...
#     webpki = ISC
LICENSE="Apache-2.0 MIT 0BSD Apache-2.0 Apache-2.0-with-LLVM-exceptions Artistic-2 BSD BSD-2 Boost-1.0 CC0-1.0 ISC MIT MPL-2.0 Unicode-DFS-2016 Unlicense ZLIB openssl SSLeay"
SLOT="0"
KEYWORDS="~amd64"
BDEPEND="
	dev-util/gn
...
"
PATCHES=(
	"${FILESDIR}/deno-1.28.2-fix_disable_tests.patch"
)
DOCS=(Releases.md)
function find_crate() {
...
src_compile() {
	...
	if use test;then
		pushd "${S}/test_util" > /dev/null || die "Failed to change dir to test_util"
		cargo_src_compile --locked
		popd > /dev/null
	fi
...
pkg_postinst(){
	if ! tc-is-clang ; then
		...
		#TODO unbundle third party libs(ICU) if possible
		:
	fi
}
