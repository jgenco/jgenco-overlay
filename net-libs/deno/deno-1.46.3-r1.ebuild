...
"
PATCHES=(
	"${FILESDIR}/deno-1.46.3-fix_disable_tests.patch"
)
...
pkg_pretend() {
	#This used 8.7GB using 9G for safety
...
src_prepare() {
	...
	eapply -d "${ECARGO_VENDOR}/$(find_crate ^v8@)" -- \
		...
		"${FILESDIR}/v8-0.105.0-v8-out_dir.patch" \
	...
	sed -i -e '/if (is_clang && !is_nacl && !is_cronet_build) {/,+2d' \
		"${ECARGO_VENDOR}/$(find_crate ^v8@)/build/config/compiler/BUILD.gn" || die

	#https://github.com/denoland/rusty_v8/issues/1145 - only for testing?
	ln -s "${ECARGO_VENDOR}/$(find_crate deno_core_icudata)/src" \
		"${ECARGO_VENDOR}/$(find_crate ^v8@)/third_party/icu/common" || die
	#v8 crate end

	eapply -d "${ECARGO_VENDOR}/$(find_crate libffi-sys)/libffi" -- \
		"${FILESDIR}/libffi-sys-2.3.0-trampoline-c99.patch"
