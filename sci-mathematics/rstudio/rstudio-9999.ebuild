...
LICENSE="AGPL-3 MIT Apache-2.0 panmirror? ( AGPL-3 MIT ISC Apache-2.0 BSD Unlicense CC0-1.0  || ( AFL-2.1 BSD ) || ( WTFPL-2 MIT ) CC-BY-3.0 LGPL-3.0 Python-2.0 || ( MIT X11 ) )"
...
src_prepare(){
	...
	if  use panmirror;then
		if [[ ${PANMIRROR_LOCK_HASH} != ${PANMIRROR_SRC_HASH:0:40} ]];then
			die "Panmirror Hash doesn't match"
		else
			patch -d ${WORKDIR}/node_gyp -p1 < ${FILESDIR}/node-gyp-9.0.0-remove-devDep.patch || die "Node-gyp patch failed"
		fi
	fi
}
src_configure() {
