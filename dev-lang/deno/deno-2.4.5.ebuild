...
#if no work disable tests/node_compat/test_runner.rs
NODE_TEST_HASH="efdcff0c97da58383af5725d3d68cf653161c73f"
V8_VER="137.2.1"
...
SRC_URI+="
	...
	sys-libraries? (
		https://github.com/chromium/chromium/raw/dfd88085261b662a5c0a1abea1a3b120b08e8e48/tools/generate_shim_headers/generate_shim_headers.py ->
		generate_shim_headers-dfd88085.py
	)
"
LICENSE="MIT"
LICENSE+="
...
PATCHES=(
	...
	"${FILESDIR}/deno-2.4.5-fix_disable_tests.patch"
)
...
disk_space_check() {
	...
	if ! use v8-prebuilt; then
		disk_space="$(usex llvm 7 10)"
		disk_space_test="$(usex llvm 7 14)"
	fi
...
src_unpack() {
	...
	if use test; then
		...
		mv "${WORKDIR}/std-release-${DENO_STD_VER}/" \
...
src_prepare() {
	...
	#from https://gitlab.alpinelinux.org/alpine/aports/-/blob/master/community/deno/
	if use sys-libraries; then
		eapply "${FILESDIR}/deno-2.4.5-cargo.patch"
		pushd "$(find_crate ^v8- p)" || die
		eapply "${FILESDIR}/v8-137.2.1-system-libraries.patch"
		local use_system="icu" # simdutf"
		install -Dm755 "${DISTDIR}/generate_shim_headers-dfd88085.py" \
			tools/generate_shim_headers/generate_shim_headers.py || die
		build/linux/unbundle/replace_gn_files.py --system-libraries ${use_system} || die
		for lib in ${use_system}; do
			find . -type f -path "*third_party/${lib}/*" \
				\! -regex '.*\.\(gn\|gni\|isolate\|py\)' -delete || die
		done
		popd
		PATCHES+=("${FILESDIR}/deno-2.4.5-unbundle-ca-certs.patch")
	else
	...
	sed -Ei 's/(".*(dns|test-http-https-default-ports).*)/#\1 #need internet/'\
		tests/node_compat/config.toml || die
	...
	use test && ( sed -i "s/0.25.5/$(esbuild-0.25 --version)/" tests/util/server/src/npm.rs || die )
	default
}
